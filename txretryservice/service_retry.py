...
class RetryingCall(service.Service):
    """
    ...
    def _err(self, fail):
        ...
        if fail.check(defer.CancelledError):
            # This cancelled error will get handled by
            # the errback handler for cancelling
            return fail
    ...
    def _nextDelayCall(self):
        """
        After the next delay amount, call our function.
        """
        if not self.running:
            logging.info("Stopping retrying function on %s" % self._func)
            ...
            try:
                ...
                self._function_deferred.errback(self.failures[0] if self.failures else None)
            else:
                self._iterator_deferred = task.deferLater(reactor, delay,
                                                          self._func, *self._args, **self._kw)
                self._iterator_deferred.addCallbacks(self._function_deferred.callback, self._err)

    def configureService(self, backoffIterator=None, failureTester=None):
        """
        ...
        # function_deferred will callback once requested call successfully complete
        self._function_deferred = defer.Deferred()
        return self._function_deferred
        
    def stopService(self):
        service.Service.stopService(self)
        if self._iterator_deferred and not self._iterator_deferred.called:
            # Swallow the CancelledError that will be thrown
            # when we cancel it
            self._iterator_deferred.addErrback(lambda r: None)
            self._iterator_deferred.cancel()
